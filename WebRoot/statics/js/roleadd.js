...
$(function(){
	...
	roleCode.bind("blur",function(){
		...
		//显示友情提示
		validateTip(roleCode.next(),{"color":"#666666"},"* 角色编码长度必须是大于1小于20的字符",false);
	});
	
	roleName.bind("focus",function(){
		validateTip(roleName.next(),{"color":"#666666"},"* 角色名称长度必须是大于1小于20的字符",false);
	}).bind("blur",function(){
		if(roleName.val() != null && roleName.val().length > 1
				&& roleName.val().length < 20){
